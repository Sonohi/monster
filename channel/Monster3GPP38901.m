	...
	methods
		...
		function [receivedPower, receivedPowerWatt] = computeLinkBudget(obj, Station, User, mode, varargin)
			...
			if isempty(varargin)
				...
				switch mode
				case 'downlink'
					lossdB = obj.computePathLoss(Station, User, Station.Tx.Freq, d2, d3, positions);
					receivedPower = EIRPdBm-lossdB-DownlinkUeLoss; %dBm
				case 'uplink'
					lossdB = obj.computePathLoss(Station, User, User.Tx.Freq, d2, d3, positions);
					EIRPdBm = User.Tx.getEIRPdBm;
					receivedPower = EIRPdBm-lossdB-Station.Rx.NoiseFigure; %dBm
				end
			end
		...
		function [lossdB] = computePathLoss(obj, TxNode, RxNode, Freq, varargin)
			...
			if isempty(varargin) %if not empty distance2d and distance 3d is defined by input
				...
				distance2d = varargin{1};
				distance3d = varargin{2};
			end

			areatype = obj.Channel.getAreaType(TxNode);
			seed = obj.Channel.getLinkSeed(RxNode, TxNode);
			[LOS, prop] = obj.Channel.isLinkLOS(TxNode, RxNode, false, distance2d);
			if ~isnan(prop)
				...
				if isempty(varargin)
					...
					LOS = obj.spatialLOSstate(TxNode, varargin{3}, prop);
				end
			end
			...
			if isempty(varargin)
                ...
                if shadowing
                    ...
                    XCorr = arrayfun(@(x,y,z) obj.computeShadowingLoss(TxNode, [x y], z), reshape(varargin{3}(:,1),size(LOS)), reshape(varargin{3}(:,2),size(LOS)), LOS );
                    lossdB = lossdB +XCorr
                end
			end		
		end
