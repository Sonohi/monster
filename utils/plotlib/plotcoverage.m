function plotcoverage(stations,channel, Config)
	...
	for stationIdx = 1:length(stations)
		...
		if exist(filename, 'file') && ~Config.SimulationPlot.generateCoverageMap
			...
			coverage = computeCoverage(station,channel, Config);
		...
		hCoverage = rectangle(Config.Plot.LayoutAxes,'Position', [px py d d], 'Curvature', [1,1],'FaceColor',color,'EdgeColor','none','Tag',strcat('coverage',num2str(stationIdx)));
