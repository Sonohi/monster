...
if numel(h_track) > 1
    ...
    n_tx = size( h_track(1,1).scenario ,1);
    
    if create_segments
                
        [ trk_length , dist ] = h_track(1,1).get_length;
        no_snapshots = h_track(1,1).no_snapshots;
        ...
        while pos < trk_length
            ...
            if ind < no_snapshots     % Exception for the last segment
                no_segments = h_track(1,1).no_segments + 1;
                h_track(1,1).no_segments = no_segments;
                h_track(1,1).segment_index( no_segments ) = ind;
                
                if ~isempty( scenario )
                    for i_tx = 1:n_tx
                        i_scen = find( rand < cum_probability,1 );
                        h_track(1,1).scenario{ i_tx,no_segments-1 } = scenario{i_scen};
                    end
                end
            end
        end
        ...
        if ~isempty( scenario )
            for i_tx = 1:n_tx
                i_scen = find( rand < cum_probability,1 );
                h_track(1,1).scenario{ i_tx,no_segments } = scenario{i_scen};
            end
        end
        ...
        if ~isempty( scenario )
            for i_segment = 1:h_track(1,1).no_segments
                for i_tx = 1:n_tx
                    i_scen = find( rand < cum_probability,1 );
                    h_track(1,1).scenario{ i_tx,i_segment } = scenario{i_scen};
                end
            end
        end
    end
