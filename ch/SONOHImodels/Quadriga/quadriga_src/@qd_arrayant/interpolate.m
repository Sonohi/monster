...
% Output:
%   V
%   The interpolated vertical field pattern (θ-component)
%
%   H
%   The interpolated horizontal field pattern (ϕ-component)
%
%   dist
...
if nargin < 5
    ...
    if use_single_precision
        ...
        Fa = single( h_qd_arrayant.Fa );
        Fb = single( h_qd_arrayant.Fb );
...
V = zeros( no_values, no_element, precision );
H = zeros( no_values, no_element, precision );
...
pX = [pa,pb,pc,pd].';
pY = uint32( (i_element-1)*no_az*no_el );
