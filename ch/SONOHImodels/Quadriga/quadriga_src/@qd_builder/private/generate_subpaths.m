...
switch h_builder.scenpar.SubpathMethod
    ...
    case 'mmMAGIC'
        ...
        SC_lambda = h_builder.scenpar.SC_lambda;        % Spatial consistency decorrelation distance in [m]
        ...
        for n = nLOS+1 : n_clusters
            ...
            if SC_lambda == 0
                ...
                randC = qd_sos.rand( ones(1, M ) * SC_lambda , h_builder.rx_positions ).';
            end
            ...
            ao = 1:floor( (M+0.1)/2 );
            if ao(end) > M/2-0.1
                ao = [ -log(2*ao(end:-1:1)), log(2*ao) ] ;
            else
                ao = [ -log(2*ao(end:-1:1)), 0, log(2*ao) ];
            end
