...
if numel(h_builder) > 1
    ...
    if h_builder.no_rx_positions > 0 && ( size( h_builder.pow,3 ) ~= n_freq || ~h_builder.data_valid  )
        split_ssf = false;
    end
    ...
    for f = 1 : n_freq
        ...
        if h_builder.no_rx_positions ~= 0
            ...
            if split_ssf
                ...
                h_builder_out(1,f).xpr_path     = h_builder.xpr_path(:,:,f);
                ...
                if ~isempty( h_builder.random_pol )
                    h_builder_out(1,f).random_pol = h_builder.random_pol(:,:,:,f);
                end
            ...
            % Split the track objects including
            rx_track = qd_track;
            for t = 1 : size( h_builder.rx_track , 2 )
                if isempty( h_builder.rx_track(1,t).par )
                    ...
                    rx_track(1,t).par_nocheck = data;
                end
            end
            h_builder_out(1,f).rx_track = rx_track;
            
            h_builder_out(1,f).data_valid = split_ssf;
        end
    end
end
