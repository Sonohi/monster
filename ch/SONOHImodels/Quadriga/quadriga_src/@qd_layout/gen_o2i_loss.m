function o2i_loss_dB = gen_o2i_loss( h_layout, method, low_loss_fraction, SC_lambda, max_indoor_distance )
...
%   then stored with the tracks in 'qd_layout.track.par.o2i_loss' and
%   'qd_layout.track.par.o2i_d3din'. These two variables are then read again by 'qd_builder.get_pl'
...
%   in 'qd_layout.track.par.o2i_loss'. Each cell contains an array of values, where the dimensions
...
% Spatial consistency variable
if ~exist( 'max_indoor_distance', 'var' ) || isempty( max_indoor_distance )
    max_indoor_distance = 25;
end
...
% Create an index of all users and segments in the layout
i_rx   = [];
i_seg  = [];
for r = 1 : no_rx
    no_seg = h_layout.track(1,r).no_segments;
    i_rx   = [ i_rx, ones(1,no_seg)*r ];
    i_seg  = [ i_seg, 1:no_seg ];
    if size( h_layout.track(1,r).scenario ) ~= h_layout.track(1,r).no_segments
        error('QuaDRiGa:qd_layout:gen_o2i_loss','??? scenario is not defined')
    end
end

% Parse the positions
pos = zeros( 3, numel(i_rx) );             % The MT positions in 3D coordinates
for r = 1 : numel( i_rx )
    segment_index    = h_layout.track(1,i_rx(r)).segment_index(1,i_seg(r));
    initial_position = h_layout.track(1,i_rx(r)).initial_position;
    track_position   = h_layout.track(1,i_rx(r)).positions(:,segment_index );
    pos( :,r )       = initial_position + track_position;
end

% Select Low-loss or High-Loss model
use_low_loss  = false( 1, numel(i_rx) );
use_high_loss = false( 1, numel(i_rx) );

% Generate spatially correlated uniform random variable for the "low/high loss selection"
if SC_lambda == 0
    randC = rand( 1, numel(i_rx) );
else
...
PL_tw = zeros( no_tx, numel(i_rx), no_freq );
for f = 1 : no_freq
    for t = 1 : no_tx
...
d_3d  = zeros( no_tx, numel(i_rx) );    % 3D distance
d_2d  = zeros( no_tx, numel(i_rx) );    % 2D distance
theta = zeros( no_tx, numel(i_rx) );    % Elevation angle
for t = 1 : no_tx
    % Vector pointing from the BS to the MT
    pos_tmp = pos - h_layout.tx_position(:,t) * ones( 1,numel(i_rx) );
...
d_2d_in = d_2d_in * max_indoor_distance;
...
switch method
    case 'mmMAGIC'
        ...
        PL_el = PL_el(:,:,o_frq);
    otherwise
        PL_el = zeros( no_tx, numel(i_rx), no_freq );
end

% Standard sevialtion of the penetration-loss
switch method
    case '3GPP_38.901'
...
for r = 1 : no_rx
    ...
    if any( abs( pl_tmp(:) ) ~= 0 )
        h_layout.track(1,r).par.o2i_loss = PL( :, i_rx == r, : );
        h_layout.track(1,r).par.o2i_d3din = d_3d_in(:,i_rx == r);
    end
end
